...
for(i in 1:length(batchList)){
  ...
  inFile2 = paste("~/Genexp_25Nov2013/brainSeq.singleCell", batchName, "wholeGene.geneComposite.gene.count.txt",sep=".");
...
for (i in c("3000", "2000")){
  ...
  no.MT.Variance <- Variance[-which(rownames(Variance)%in%MTGenes),]  #Mitochondria genes removed
  
  Top.Var <- no.MT.Variance[1:i,]
  Top.Var <- recRPKM[rownames(Top.Var),]
    
  clustPv <-parPvclust(cl, Top.Var, method.dist="cor", method.hclust="complete", nboot=10000) 
  save(clustPv, file = paste(Sys.Date(), "_pvclust_Top_", i, "_var_genes_NO_MT_Genes.Rdata", sep = ""))
  }
